...
function ModPlayer(mod, rate, rowCallback, posCallback, midiCallback) {
	/* timing calculations */
	...
	var modPlayerCallback = this;
	var rowMove = function (row) { rowCallback(row, modPlayerCallback); }
	var positionMove = function (pos) { posCallback(pos, modPlayerCallback); }
  var triggerMidi = function(row, chan, note) { midiCallback(row, chan, note); }
	...
	var channels = [];
	var disabledChannels = new Array();
	...
	function loadRow(rowNumber) {
		...
		rowMove(currentRow);
		for (var chan = 0; chan < mod.channelCount; chan++) {
			if (disabledChannels.indexOf(chan) == -1) {
				var note = currentPattern[currentRow][chan];
				if (note.period != 0 || note.sample != 0) {
          triggerMidi(currentRow, chan, ModPeriodToNoteNumber[note.period]);
	...
	this.toggleChannel = function (chan) {
		if (disabledChannels.indexOf(chan) == -1)
			disabledChannels.push(chan);
		else
			disabledChannels.splice(disabledChannels.indexOf(chan), 1);
		return (disabledChannels.indexOf(chan) == -1);
	}

	this.getPatternAtPosHtml = function (pos) {
	    ...
	    for (var chanLoop = 0; chanLoop < thePattern[0].length; chanLoop++) {
	        ...
	        for (var rowLoop = 0; rowLoop < thePattern.length; rowLoop++) {
	            note = thePattern[rowLoop][chanLoop];
	            noteSample = note.sample.toString(16).toUpperCase();
	            if (noteSample.length == 1)
	                noteSample = '0' + noteSample;
	            noteEffect = note.effect.toString(16).toUpperCase();
	            noteEffectParam = note.effectParameter.toString(16).toUpperCase();
	            if (noteEffectParam.length == 1)
	                noteEffectParam = '0' + noteEffectParam;
	            retVal += '<span id="chan' + chanLoop + 'row' + rowLoop.toString() + '" class="PatternDiv' + this.getNoteString(ModPeriodToNoteNumber[note.period]) + '">' + this.getNoteAndOctString(ModPeriodToNoteNumber[note.period]) + '<span class="NoteEffect">' + noteSample + ' ' + noteEffect + ' ' + noteEffectParam + '</span></span>';
	        }
	        retVal += '</div>';
	    }
	    retVal += '</div>';
	...
	this.getNoteAndOctString = function (note) {
		if (note) {
			var noteNum = note % 12;
	...
	this.getNoteString = function (note) {
		if (note) {
			var noteNum = note % 12;
			var code = null;
			switch (noteNum) {
	...
	this.getSamples = function (sampleCount) {
		...
		while (i < sampleCount) {
			...
			for (var chan = 0; chan < mod.channelCount; chan++) {
				if (channels[chan].playing) {
					...
					while (channels[chan].ticksSinceStartOfSample >= channels[chan].ticksPerSample) {
						...
						//console.log('RepLen : ' + channels[chan].sample.repeatLength + ', RepOffset : ' + channels[chan].sample.repeatOffset + ', samplePos : ' + channels[chan].samplePosition);
						if (channels[chan].sample.repeatLength > 2 && channels[chan].samplePosition >= channels[chan].sample.length) {
			...
			leftSamples[i] = leftSamples[i + 1] = leftOutputLevel / (128 * 128 * mod.channelCount);
			rightSamples[i] = rightSamples[i + 1] = rightOutputLevel / (128 * 128 * mod.channelCount);
			i += 2;
		}

		return { samples: samples, leftSamples: leftSamples, rightSamples: rightSamples };
	}
}
